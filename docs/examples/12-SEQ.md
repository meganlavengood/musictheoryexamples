---
layout: examples-list
title: "Sequences"
date: 2024-11-15 15:02:18 -0500
---

## Descending fifths, 6/3 variant, etc.

<table class="tablesaw tablesaw-stack" data-tablesaw-mode="stack">
  <thead>
    <tr>
      <th>.mp3</th>
      <th>.pdf</th>
      <th>Example</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td><a href="12-seq/SEQa.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQa.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Schubert, Impromptu in E-flat, D. 899 no. 2, mm. 25-35<strong></strong></td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQb.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQb.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Rossini, William Tell, Overture, mm. 115-123</td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQc.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQc.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Paganini, Caprice for Solo Violin, op. 1 no. 24, mm. 1-12</td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQd.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQd.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Haydn, Symphony no. 1 in D Major, mvt. 2, mm. 37-45</td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQe.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQe.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Corelli, Concerto Grosso in F Major, op. 6 no. 12, mvt. 5, mm. 73-78</td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQf.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQf.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Saint-Sa&euml;ns, Piano Concerto no. 2 in g minor, op. 22, mvt. 3, mm. 293-303</td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQg.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQg.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Mozart, Piano Concerto no. 21 in C Major, K. 467, mvt. 1, mm. 147-154</td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQh.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQh.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Mozart, Mass in c minor, K. 427, Quoniam, mm. 139-147</td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQi.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQi.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Mozart, Piano Sonata in F Major, K. 332, mvt. 1, mm. 192-203</td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQj.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQj.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Schumann, Kreisleriana, op. 16 no. 7, mm. 9-32</td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQk.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQk.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>J.S. Bach, Sonata no. 1 in g minor for Solo Violin, BWV 1001, mvt. 4, mm. 7-16</td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQl.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQl.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Rodrigo, Fantasia para un gentilhombre, mvt. 2, mm. 21-28</td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQm.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQm.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Brahms, Intermezzo, op. 117 no. 2, mm. 1-9</td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQn.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQn.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Brahms, Clarinet Sonata in E-flat Major, op. 120 no. 2, mvt. 1, mm. 161-166 </td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQo.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQo.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Brahms, Piano Concerto no. 2 in B-flat Major, op. 83, mvt. 1, mm. 146-153 </td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQp.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQp.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Haydn, String Quartet in C Major, op. 20 no. 2, mvt. 3, mm. 57-68</td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQq.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQq.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Mozart, Piano Sonata in C Major, K. 545, mvt. 1, mm. 61-67</td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQr.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQr.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Mozart, Clarinet Quintet, K. 581, mvt. 2, mm. 33-40</td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQs.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQs.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Grieg, Holberg Suite, op. 40, mvt. 4, mm. 16-21</td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQt.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQt.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Vivaldi, Violin Concerto in a minor, op. 3 no. 6, mvt. 2, mm. 1-6
        (sequence avoided) </td>
    </tr>

  </tbody>
</table>

## Ascending fifths

<table class="tablesaw tablesaw-stack" data-tablesaw-mode="stack">
  <thead>
    <tr>
      <th>.mp3</th>
      <th>.pdf</th>
      <th>Example</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td><a href="12-seq/SEQu.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQu.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>J. S. Bach, Prelude in C Major, BWV 924, mm. 1-5</td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQv.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQv.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Corelli, Concerto Grosso in C Major, op. 6 no. 10, mvt. 4, mm. 1-9</td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQw.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQw.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Brahms, Symphony no. 4 in e minor, op. 98, mvt. 1, mm. 4-9</td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQy.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQy.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Rachmaninoff, Rhapsody on a Theme of Paganini, op. 43, Variation 18, mm. 5-12</td>
    </tr>

  </tbody>
</table>

## Descending 5-6 and root-position variant

<table class="tablesaw tablesaw-stack" data-tablesaw-mode="stack">
  <thead>
    <tr>
      <th>.mp3</th>
      <th>.pdf</th>
      <th>Example</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td><a href="12-seq/SEQa1.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQa1.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Chopin, Etude in G-Flat Major, op. 25 no. 9, mm. 1-8</td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQb1.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQb1.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Haydn, Cello Concerto in D Major, mvt. 2, mm. 23-28</td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQc1.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQc1.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Mozart, The Magic Flute, Act II, Queen of the Night Aria, mm. 24-32</td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQd1.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQd1.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Mozart, Violin Sonata in G Major, K. 379, mvt. 3, mm. 1-8</td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQe1.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQe1.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Schubert, Symphony no. 9 in C Major, D. 944, mvt. 2, mm. 112-116</td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQf1.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQf1.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Beethoven, Piano Sonata in C Major, op. 53, mvt. 1, mm. 1-13</td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQh1.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQh1.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Pachelbel, Canon in D, mm. 1-8</td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQj1.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQj1.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Corelli, Concerto Grosso in g minor, op. 6 no. 8, mvt. 3, mm. 3-4</td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQk1.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQk1.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Chopin, Etude in E Major, op. 10 no. 3, mm. 16-21</td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQl1.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQl1.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Poulenc, Gloria, Domine Deus, mm. 1-4</td>
    </tr>

  </tbody>
</table>

## Ascending 5-6, etc.

<table class="tablesaw tablesaw-stack" data-tablesaw-mode="stack">
  <thead>
    <tr>
      <th>.mp3</th>
      <th>.pdf</th>
      <th>Example</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td><a href="12-seq/SEQm1.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQm1.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Chopin, Piano Concerto no. 1 in e minor, op. 11, mvt. 3, mm. 471-477</td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQo1.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQo1.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Vivaldi, Violin Concerto in f minor, op. 8 no. 4 (&quot;Winter&quot;), mvt. 2, mm. 11-13</td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQp1.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQp1.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Corelli, Concerto Grosso in F Major, op. 6 no. 2, mvt. 4, mm. 41-46</td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQq1.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQq1.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Handel, Concerto Grosso, op. 6 no. 1, mvt. 4, mm. 49-55</td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQr1.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQr1.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Mozart, Piano Concerto no. 23 in A Major, K. 488, mvt. 1, mm. 258-262</td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQs1.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQs1.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>J.S. Bach, Keyboard Partita no. 4 in D Major, BWV 828, Allemande, mm. 18-21</td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQu1.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQu1.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Corelli, Concerto Grosso in F Major, op. 6 no. 2, mvt. 1, mm. 87-93</td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQv1.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQv1.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Corelli, Concerto Grosso in F Major, op. 6 no. 6, mvt. 4, mm. 120-125</td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQw1.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQw1.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>J.S. Bach, Brandenburg Concerto no. 4 in G Major, BWV 1049, mvt. 1, mm. 47-53</td>
    </tr>

  </tbody>
</table>

## Others

<table class="tablesaw tablesaw-stack" data-tablesaw-mode="stack">
  <thead>
    <tr>
      <th>.mp3</th>
      <th>.pdf</th>
      <th>Example</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td colspan="5"><em>Aldwell/Schachter refer to these sequences as &quot;up
          a step with voice-leading chords&quot;)</em></td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQx1.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQx1.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Beethoven, Piano Concerto no. 5 in E-flat Major, op. 73, mvt. 2, mm. 20-25</td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQy1.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQy1.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Mozart, Piano Concerto no. 14 in E-flat Major, K. 449, mvt. 2, mm. 89-97 </td>
    </tr>
    <tr>
      <td><a href="12-seq/SEQz1.pdf">{% octicon file height:16 aria-label:pdf %}</a></td>
      <td><a href="12-seq/SEQz1.mp3">{% octicon unmute height:16 aria-label:mp3 %}</a></td>
      <td>Beethoven, Violin Sonata no. 1 in D Major, op. 12 no. 1, mvt. 3, mm. 157-165 </td>
    </tr>

  </tbody>
</table>
